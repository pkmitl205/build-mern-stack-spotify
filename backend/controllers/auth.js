...
exports.logout = catchAsync(async (req, res, next) => {
  ...
  res.status(StatusCodes.OK).json({ status: 'success', message: '✌️ See you soon!' });
});
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError('🔐 You are not logged in! Please log in to access', StatusCodes.UNAUTHORIZED)
    );
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError('🚫 Token is invalid or expired', StatusCodes.BAD_REQUEST));
  }
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.checkPassword(req.body.currentPassword, user.password))) {
    return next(new AppError('🔐 Your password is incorrect', StatusCodes.UNAUTHORIZED));
  }
